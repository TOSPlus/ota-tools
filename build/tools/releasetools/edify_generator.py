...
class EdifyGenerator(object):
  ...
  def AssertDevices(self, devices):
    ...
    cmd = ( ' || '.join(['getprop("ro.product.device") == "%s" || getprop("ro.build.product") == "%s"'
                % (i, i) for i in devices.split(",")]) + ' || abort("This package is not for this device");')
    self.script.append(cmd)

  def AssertDevice(self, device, product):
    """Assert that the device identifier is the given string."""
    cmd = ('getprop("ro.product.device") == "%s" || '
  ...
  def AddToZip(self, input_zip, output_zip, input_path=None):
    ...
    if input_path is None:
      try:
    	  data = input_zip.read("OTA/bin/file_contexts")
    	  common.ZipWriteStr(output_zip, "META-INF/com/google/android/file_contexts",
                       data, perms=0755)
      except:
        print "no need package file_contexts"
    elif os.path.exists(os.path.join(input_path, "file_contexts")):
      try:
        data = open(os.path.join(input_path, "file_contexts")).read()
        common.ZipWriteStr(output_zip, "META-INF/com/google/android/file_contexts",
                       data, perms=0755)
      except:
        print "no need package file_contexts input_path"
