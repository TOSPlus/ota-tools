...
OPTIONS = Options()
OPTIONS.search_path = os.path.join(os.environ["PORT_ROOT"], "build/tools");
OPTIONS.signapk_path = "signapk.jar"  # Relative to search_path
...
def BuildBootableImage(sourcedir, fs_config_file, info_dict=None):
  ...
  for arg_item in ["base", "kernel_offset", "ramdisk_offset", "second_offset", "tags_offset", "cmdline", "pagesize"]:
    fn = os.path.join(sourcedir, arg_item)
...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir,
  ...
  if os.path.exists(prebuilt_path):
    ...
    # Currently, most QROM does not has recovery.img
    ...
    print "This QROM does not contain recovery.img"
    return None
...
def SignFile(input_name, output_name, key, password, align=None,
  ...
  cmd = [OPTIONS.java_path, "-Xmx4096m", "-jar",
...
class PasswordManager(object):
  def __init__(self):
    ...
    self.qromrp = os.getenv("QROM_RANDOM_PASS", None)
  ...
  def PromptResult(self, current):
    ...
    for k, v in sorted(current.iteritems()):
      if v:
        ...
        while True:
          if self.qromrp: result[k] = self.qromrp
          else: result[k] = "m/EgNsm2iEFLNSGpgEMtyA=="
          if result[k]: break
    return result
